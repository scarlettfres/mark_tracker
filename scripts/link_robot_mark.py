...
class link_head_robot:
    ...
    def __init__(self, body_part, id_mark, x, y, path):
        ...
        # que se passe t il si la marque disparait pendant plus d'un temps t ?
        # un message d'erreur apparait et on arrete de publier les tf
        self.clock_verif_erreur = rospy.Time.now(
        ) + rospy.Duration(temps_erreur)
        # pour caler la clock
        rospy.Subscriber(
    ...
    <node pkg="tf" type="static_transform_publisher"
        ...
        message = message + str(self.trans_markeur_to_body[0]) + " " + str(self.trans_markeur_to_body[1]) + " " + str(
            self.trans_markeur_to_body[2]) + " "    # transaltion puis rotation (attention pas le meme ordre dangles)
        message = message + str(self.rot_markeur_to_body[0]) + " " + str(
            self.rot_markeur_to_body[1]) + " " + str(self.rot_markeur_to_body[2])
    ...
    def joint_state_callback(self, data):
        if self.detection == 1:
            ...
            try:
                ...
                (trans_body_to_ref, rot_body_to_ref) = self.listener.lookupTransform(
                    self.robot_body_part, robot_ref, rospy.Time(0))
                #(trans_body_to_ref,rot_body_to_ref) = self.listener.lookupTransform("/HeadTouchFront_frame", "/base_link", rospy.Time(0))
                (trans_foot_to_body, rot_foot_to_body) = self.listener.lookupTransform(
                    robot_foot, self.robot_body_part, rospy.Time(0))
                (trans_markeur_to_map, rot_markeur_to_map) = self.listener.lookupTransform(
                    self.markeur, "map", rospy.Time(0))
                ...
                self.broadcaster.sendTransform(
                    (self.dist_x, self.dist_y, 0), (0, 0, 0, 1), now, "/mon_tf/base_footprint", "/map")
                # donc notre body part serait ici par rapport a la map
                self.broadcaster.sendTransform(
                    trans_foot_to_body, rot_foot_to_body, now, "/mon_tf/" + self.robot_body_part, "/mon_tf/base_footprint")
                ...
                (self.trans_markeur_to_body, self.rot_markeur_to_body) = self.listener.lookupTransform(
                    self.markeur, "/mon_tf/" + self.robot_body_part, rospy.Time(0))
                self.broadcaster.sendTransform(
                    self.trans_markeur_to_body, self.rot_markeur_to_body, now, "/rololo", self.markeur)
